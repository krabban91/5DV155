...
jQuery(function(){
 ...
 var displayInCurrentIframes = function(doc) {

    // find each IFrame in the current document
    jQuery(doc).find("iframe").each(function(idx, iframe) {
      try {
          // get the document associated with this IFrame
          var window = iframe.contentDocument || iframe.contentWindow.document;

          // display any Kaltura content within this IFrame 
          displayKaltura(window);

          // process any nested IFrames
          displayInCurrentIframes(window);
      }
      catch (err) {
          // Nothing we can do but swallow the error.
      }
  ...
  jQuery(document).ready(function(){
      displayKaltura(document);
      findIframes(this);
  });

  // Use this on any element containing descendant iframe(s), whenever some new html content is included
  jQuery.fn.displayKaltura = function() {
      displayInCurrentIframes(this);
  };
